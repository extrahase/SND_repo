...
FatesData = {
    ...
    },
    {
        ...
        fatesList= {
            collectionsFates= {},
            otherNpcFates= {
                { fateName="Corporal Pomishment", npcName="Mogpo the Magnificent" },
            },
            fatesWithContinuations = {},
            blacklistedFates= {}
        }
    ...
    },
    {
        ...
        fatesList= {
            ...
            otherNpcFates= {
                ...
                { fateName="Lepus Lamentorum: Crazy Contraption", npcName="Fallingway" },
            },
            fatesWithContinuations = {},
            blacklistedFates= {
...
function TurnOnAoes()
    if not AoesOn then
        yield("/vbm ar set "..RotationAoePreset)
    end
    AoesOn = true
end
...
function TurnOffAoes()
    if AoesOn then
        yield("/vbm ar set "..RotationSingleTargetPreset)
    end
    AoesOn = false
end
...
function TurnOffRaidBuffs()
    if AoesOn then
        yield("/vbm ar deactivate "..RotationHoldBuffPreset)
    end
end
...
function DoFate()
    ...
    -- if a living Forlorn is present, turn on single target rotation
    if forlorn then
        if forlorn.HealthPercent ~= 0 then
...
elseif RotationPlugin == "BossMod" and HasPlugin("BossMod") then
    ...
    -- For BMR/VBM/Wrath
    RotationSingleTargetPreset      = "Forlorn" --Preset name with single target strategies (for forlorns). TURN OFF AUTOMATIC TARGETING FOR THIS PRESET
    RotationAoePreset               = "FATE" --Preset with AOE + Buff strategies.
    RotationHoldBuffPreset          = "FATE" --Preset to hold 2min burst when progress gets to seleted %
    PercentageToHoldBuff            = 100            --Ideally you'll want to make full use of your buffs, higher than 70% will still waste a few seconds if progress is too fast.
...
MaxWait                             = 0            --Max number of seconds it should wait until mounting up for next fate.
